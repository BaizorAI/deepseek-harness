...
interface Koffi {
  ...
  decode: {
    ...
    /** Decode a NUL-terminated UTF-16 string at a native address (Koffi 3.1+). */
 ...
 * `_Out_ void **` out-params surface a raw address, and `koffi.decode`'s
 * legacy `str16` form dereferences it as a pointer — a crash on real
 * Windows — so use the Koffi 3.1 `decode.string16` memory reader, which
 * reads the string at the address itself.
 */
