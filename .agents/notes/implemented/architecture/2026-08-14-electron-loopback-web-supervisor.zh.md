...
根目录的 `package:desktop` 命令是完整的本地打包入口。它会执行同样的仓库构建，然后由 `apps/desktop/scripts/stage-runtime.ts` 根据仅含依赖的 manifest，创建已忽略的 `apps/desktop/runtime-host` 目录树。该暂存器通过 pnpm JavaScript 入口、以 `--config.allowUnusedPatches=true` 运行仅生产环境依赖的提升式 `pnpm deploy`，补回 legacy deploy 遗漏的直接依赖，将包链接实体化，并拒绝所有残留的符号链接。工作区的 Electron Builder 补丁永远不会应用到仅含依赖的 deploy 集合；入口间接寻址让 deploy 在 Windows 上保持无 shell，allowance 则记录这一经过评估的例外。Electron Builder 会把该封闭目录树与构建后的 Web 前端一起复制到应用的 `resources/host` 目录。
